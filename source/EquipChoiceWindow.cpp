	...
	for(u8 slot = 0 ; slot < 3 ; slot++) {
		...
		if(!armor) {
			for(auto it : ItemManager::armors) {
				if(it->slot() == slot) {
					it->thumbnail()->render(m_x + GameWindow::main->viewportX() + 20, m_y + GameWindow::main->viewportY() + 52 + slot * 32);
					break;
				}
			}
			Interface::defaultFont->setStyle(FONT_LARGE, TTF_STYLE_ITALIC);
			Interface::defaultFont->printScaled(_t("Empty").c_str(), m_x + GameWindow::main->viewportX() + 48, m_y + GameWindow::main->viewportY() + 52 + slot * 32, m_width - 40, 32, FONT_LARGE, Color::system);
			Interface::defaultFont->setStyle(FONT_LARGE, TTF_STYLE_NORMAL);
		} else {
			armor->thumbnail()->render(m_x + GameWindow::main->viewportX() + 20, m_y + GameWindow::main->viewportY() + 52 + slot * 32);
			Interface::defaultFont->printScaled(armor->name().c_str(), m_x + GameWindow::main->viewportX() + 48, m_y + GameWindow::main->viewportY() + 52 + slot * 32, m_width - 40, 32, FONT_LARGE);
		}
	}
}
