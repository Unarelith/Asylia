...
void EquipStatsWindow::draw(gk::RenderTarget &target, gk::RenderStates states) const {
	...
	if(m_currentItem && m_currentItem->type() != Item::Type::BasicItem) {
		if(m_currentItem->type() == Item::Type::Armor) {
			finalAtk = baseAtk;
			Armor *armor = m_actor->equipment()->armor(((Armor*)m_currentItem)->slot());
			if(armor) finalDef = m_actor->totalDef() - armor->def() + ((Armor*)m_currentItem)->def();
			else finalDef = m_actor->totalDef() + ((Armor*)m_currentItem)->def();
			...
			finalAtk = m_actor->atk() + ((Weapon*)m_currentItem)->atk();
			finalDef = baseDef;
	...
	printName(m_actor, 20, 20, m_width - 130, target, states);
	printLevel(m_actor, m_width - 90, 20, m_width - 20, target, states);
