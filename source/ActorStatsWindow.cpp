...
void ActorStatsWindow::drawActor(Actor *actor, u8 pos) {
	...
	Interface::defaultFont->printScaled(actor->name().c_str(), m_x + GameWindow::main->viewportX() + 20 + m_width / 4 * pos, m_y + GameWindow::main->viewportY() + 20, m_width / 4 - 115, 32, FONT_LARGE);
	...
	Interface::defaultFont->printToImage(to_string(actor->level()).c_str(), m_x + GameWindow::main->viewportX() + m_width / 4 * (pos + 1) - 20, m_y + GameWindow::main->viewportY() + 20, &level, FONT_LARGE);
