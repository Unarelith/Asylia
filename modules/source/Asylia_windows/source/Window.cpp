...
void Window::printStat(s16 x, s16 y, std::string statName, s32 statValue, u16 nameWidth, u16 x2) {
	...
	Interface::defaultFont->printScaled(statName.c_str(), x, y, nameWidth, 28, FONT_LARGE, Color::system);
	Interface::defaultFont->printToImage(to_string(statValue).c_str(), x2, y, &statImg, FONT_LARGE);
	
	statImg.render(statImg.posRect().x - statImg.width());
}

void Window::printName(Battler *battler, s16 x, s16 y, u16 width) {
	Interface::defaultFont->printScaled(battler->name().c_str(), x, y, width, 28, FONT_LARGE);
}

void Window::printState(Battler *battler, s16 x, s16 y, u16 width) {
	Interface::defaultFont->printScaled(battler->getStateString().c_str(), x, y, width, 28, FONT_LARGE);
}
